...
async def _show_dir(message, context, path: Path) -> None:
    ...
    for i, item in enumerate(items):
        ...
        if item.is_dir():
            ...
            lines.append(f"{i+1:2}.  📄 {name:<38}  {size_str:>8}")
    lines.append("</pre>")
    lines.append(f"<i>{len(items)} items</i>")
...
async def upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if doc is None:
        await update.message.reply_text(
            f"📤 <b>Upload a file</b>\n\nReply to a file with /upload [path]\n"
            f"Default save path: <code>{UPLOAD_DEFAULT_PATH}</code>",
            parse_mode=ParseMode.HTML
        )
        return
    ...
    await update.message.reply_text(
        f"✅ Saved to <code>{dest}</code>", parse_mode=ParseMode.HTML
    )
...
async def photo_received(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        "📷 Photo received — what should I do with it?",
        reply_markup=InlineKeyboardMarkup([[
...
async def print_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(f"🖨 Sent to printer: <b>{doc.file_name}</b>", parse_mode=ParseMode.HTML)
    except Exception as e:
