...
async def schedule_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if delay is None:
        await update.message.reply_text(
            f"can't parse time: <code>{time_str}</code>", parse_mode=ParseMode.HTML
        )
        return
    ...
    await update.message.reply_text(
        f"scheduled: <code>/{' '.join(cmd_parts)}</code>  in {_delay_str(delay)}",
        parse_mode=ParseMode.HTML
    )
...
async def babysit_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not is_authorized(update):
        return
    if not context.args:
        await update.message.reply_text("usage: /babysit &lt;command&gt;", parse_mode=ParseMode.HTML)
        return
    ...
    async def _watch():
        try:
            ...
            text = (
                f"<b>BABYSIT</b>  exit {proc.returncode}\n"
                f"<code>{cmd}</code>\n<pre>{output}</pre>"
            )
        except Exception as e:
            text = f"babysit failed: <code>{cmd}</code>\nerr: {e}"
...
async def wake_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        await update.message.reply_text(
            f"WoL packet sent: <code>{mac}</code>", parse_mode=ParseMode.HTML
        )
    except Exception as e:
